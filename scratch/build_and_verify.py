...
- In-store scaling: Multi-counter LAN topology (1 Primary node + 1–3 Worker terminals) (D03).
- Small single-store operators: Supported via Simple 2-role preset (§11) (D04).
...
- **Backend & Database**: Standardized on Python/FastAPI across store and Central nodes (D06). Central DB runs on cloud Postgres for reporting/aggregation (D07); Store Primary Node (Counter 1) runs dedicated local Postgres 16 (D08).
- **Multi-Counter LAN Topology**:
  - *Counter 1 (Primary Node)*: Single sequence authority committing to Postgres bound strictly to loopback `127.0.0.1` (D09). Store FastAPI binds to `0.0.0.0:8000` on private Gigabit/WPA3 LAN (D10), broadcasting via mDNS as `medpos-primary.local:8000` for DHCP IP churn immunity (D11).
  - *Counters 2/3 (Workers)*: Lightweight UI shells connecting to Counter 1 over LAN via REST/WebSockets with zero local databases (D12).
  - *Standby & Failover*: Counter 2 acts as warm standby receiving daily backups and continuous WAL streaming from Counter 1 (D13). Standby promotion script (`promote_to_primary.bat`) fences Counter 1 via LAN ping to prevent split-brain hazards before service launch (D14). Failover state machine deferred. `[DEFERRED → Doc 5: Standby Promotion Script & Fencing State Machine]` Promoted standby adopts sequence epoch `<STORE_CODE>-INV-YYYYMM-XXXX-F1` jumping sequence by +100,000 to eliminate invoice collisions on hardware loss (§8) (D15).
- **Local Authentication**: Salted Argon2id hashes and role mappings replicate from Central to store Postgres, issuing store-scoped JWTs (8–12h shift TTL) locally (D16) for 100% terminal auth autonomy during outages and reboots (D17). Token schemas deferred. `[DEFERRED → Doc 9: Local Auth JWT Claim Schema & Argon2id Hash Parameters]`
- **Unattended Maintenance**: Daily automated `pg_dump` to secondary drive with 7-day rolling retention (D18); background `VACUUM ANALYZE` and WAL pruning prevent disk exhaustion (D19). NSSM watchdog auto-restarts crashed services with backoff (max 3 restarts / 10 min) (D20). Configs deferred. `[DEFERRED → Doc 10: Unattended Edge Backup, Pruning & Watchdog Service Spec]`
- **Clock-Skew Defense**: Store Postgres enforces monotonic commit validation (`current_timestamp >= MAX(created_at)`), blocking transactions on dead CMOS resets (D21). Sync heartbeats check local time against Central; skew $> \pm 5$ min triggers UI warning (D22)."""

sec3 = """## 3. Core Modules
The system partitions into 13 discrete domain modules: Inventory & UOM Hierarchy, Customer Returns & Exchanges, Vendor Returns (RTV), Branch Stock Transfers, Prescription & Dispensing, Billing & Cashier Operations, Hardware & Peripherals, Multi-Store & Governance, Operational Directories, Sync Engine, User Roles, Settings & Config, and Reporting (D23). Build order deferred. `[DEFERRED → Doc 7: Core Module Dependency Graph & Build Order]`"""
...
Direct master-data writes require live Central connectivity with optimistic version locking to prevent multi-store divergence (D30). Governed records carry integer `version` incremented per Central write; edits reject on mismatch (`reason: stale`) (D33); creates reject on business key collision (`reason: duplicate`) (D34). Soft-deletion requires aggregate chain-wide physical stock to be zero (§6) (D35). Central Admin accesses governance queues via a Web portal with zero direct DB connection into store LANs (D36). Portal API contracts deferred `[DEFERRED → Doc 3: Central Admin Web Portal Endpoints]`; conflict scenarios deferred `[DEFERRED → Doc 6: Master-Data Concurrency Conflict & Partition Scenarios]`."""
...
- **Batch Packaging Immutability**: `packaging_unit`, `base_unit`, and `pack_size` lock immutably on Batch at GRN, isolating stock counts from catalog edits (D44). Column schemas deferred. `[DEFERRED → Doc 1: Batch Table UOM Field Schema & Constraints]`
- **Dual-Representation**: Pack and loose quantities derived via integer math: $\\text{Pack Qty} = \\text{Base Qty} // \\text{Pack Size}$, $\\text{Loose Qty} = \\text{Base Qty} \\pmod{\\text{Pack Size}}$ (D45).
...
- **Barcode Scanning**: Barcode scans bill 1 Packaging Unit ($1 \\times \\text{pack\\_size}$ base units); loose units require explicit input in dedicated UI field (D48).
- **Fractional Return Restocking**: Blister cavities with intact airtight foil return to active stock ($+\\text{Loose Qty}$); damaged cavities route to `quarantine-write-off` (D49).

### Near-Expiry Vendor Returns (RTV) & Alerts
Days to shelf expiry ($\\Delta t = \\text{expiry\\_date} - \\text{current\\_date}$) monitored: 90d Amber (FEFO priority), 60d Orange (RTV recommendation), 30d Red (immediate shelf quarantine) (D50). Pharmacists execute `stock-move: rtv-quarantine` to return bins, generating sequential GST Debit Notes (§8) (D51).

### Inter-Store Stock Transfers & Statutory Delivery Challans
Branch road transit requires Rule 55 Delivery Challans (D52) for intra-state moves with identical GSTIN; inter-state transfers mandate IGST Tax Invoices (§8) (D53). Receiving stores log `transfer-receive` splitting into `received_qty` (active), `transit_breakage_qty` (quarantine write-off with photo), and `transit_shortage_qty` (shrinkage audit) (D54). Dispatch decrements source stock; receive increments destination stock (D55). State workflows deferred. `[DEFERRED → Doc 5: Batch Picking, Fractional Return Inspection & RTV State Machines]`
...
- **Soft-Delete & Partitions**: Products cannot be soft-deleted while chain stock $> 0$ (D35). Central accepts offline sales of soft-deleted items (D63), auto-resurrecting items (`deleted_at = null`) if partition sales reveal remaining stock (D64). Scenario workflows deferred. `[DEFERRED → Doc 6: Partition Ghost-Stock Ingestion & Auto-Resurrection Scenarios]`"""
...
sec9 = """## 9. Discounts
Centrally governed discount presets reside on item or category records (D76). Cashiers are restricted to selecting pre-configured presets; manual overrides are prohibited (D77). Managers (Full preset) or High-access users (Simple preset) edit presets directly via live Central connection with optimistic version locking (§4) (D78), logged immutably as `discount-edit` events (D79). Live edit flows deferred. `[DEFERRED → Doc 5: Live Discount Edit & Version Verification Flow]`
...
Permission matrix deferred. `[DEFERRED → Doc 4: Comprehensive Role Permission Matrix (4-Role & 2-Role)]`
...
Cashiers share counters across shifts; shift unlocks upon cashier login and opening float entry (`shift-open`) (D102), tracking Cash, Card, UPI, and Note tenders.
...
checks["9. Store vs Central split explicit"] = (
    "100% autonomously offline" in full_content and "aggregation/reporting DB, never authoritative" in full_content,
