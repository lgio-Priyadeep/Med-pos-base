...
# Build fully disciplined, compact architecture doc
...
Store nodes operate 100% autonomously offline for billing, dispensing, returns, challans, stock moves, shifts, and local registration without Central connectivity (D24). Counter 1 commits all transactions to Postgres inside append-only event tables (§5) (D25); worker terminals bill against Counter 1 over LAN.
...
Direct master-data writes require live Central connectivity with optimistic version locking to prevent multi-store catalog divergence (D30). Governed records carry integer `version` incremented on each Central write; edits reject on mismatch (`reason: stale`) (D33); creates reject on business key collision (`reason: duplicate`) (D34). Soft-deletion additionally requires aggregate chain-wide physical stock to be zero (§6) (D35). Central Admin accesses governance queues via a central Web portal with zero direct DB connection into store LANs (D36). Portal API contracts deferred `[DEFERRED → Doc 3: Central Admin Web Portal Endpoints]`; conflict scenarios deferred `[DEFERRED → Doc 6: Master-Data Concurrency Conflict & Partition Scenarios]`."""
...
# Section 19: Deferred Items Register (1 line per marker, exact 1-to-1 match)
...
with open("scratch/final_compact.md", "w", encoding="utf-8") as f:
    f.write(full_content)

words = len(full_content.split())
print(f"FINAL COMPACT WORD COUNT: {words}")
