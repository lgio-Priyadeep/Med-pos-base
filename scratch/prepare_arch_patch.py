...
content = """# Pharmacy POS — Architecture Spec (v1.6.5)
...
words = content.split()
print("Draft word count:", len(words))

# Check markers
sec19 = content[content.find("## 19. Deferred Items Register") :]
body = content[: content.find("## 19. Deferred Items Register")]

body_markers = re.findall(r"\[DEFERRED → Doc \d+:[^\]]+\]", body)
reg_markers = re.findall(r"\[DEFERRED → Doc \d+:[^\]]+\]", sec19)
...
# Check D-IDs
d_tags = re.findall(r"\(D-\d+\)", content)
...
with open("scratch/arch_draft.md", "w", encoding="utf-8") as f:
    f.write(content)
print("Draft written to scratch/arch_draft.md")
